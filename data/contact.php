...
if (empty($action)) {
	...
	<div class='contact-content'>
		<h1 class='contact-title'>Отправьте сообщение:</h1>
		...
		<form action='#' style='display:none'>
			...
			<input type='text' id='contact-email' class='contact-input' placeholder='*Адрес...' name='email' tabindex='1002' />";

	if ($extra["form_subject"]) {
		$output .= "
			<!--label for='contact-subject'>Тема:</label-->
			<input type='text' id='contact-subject' class='contact-input' placeholder=' Тема...' name='subject' value='' tabindex='1003' />";
	}

	$output .= "
			<!--label for='contact-message'>*Сообщение:</label-->
			<textarea id='contact-message' class='contact-input' placeholder='*Сообщение...' name='message' cols='40' rows='4' tabindex='1004'></textarea>
			<br/>";
/*
	if ($extra["form_cc"]) {
		$output .= "
...
*/
	...
	@mail($to, $subject, $body, $headers) or 
		die("К сожалению, проблема сервера не позволила организовать доставку вашего сообщения.");
}
